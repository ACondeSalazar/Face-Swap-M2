...
def load_model(model_path, name, device, return_checkpoint=False):
    print('=> Loading %s model: "%s"...' % (name, os.path.basename(model_path)))
    checkpoint = torch.load(model_path, map_location=device)  # Ajout de map_location=device
    model = obj_factory(checkpoint['arch']).to(device)
