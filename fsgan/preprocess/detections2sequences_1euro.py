...
def main(input_path, cache_path=None, output_path=None, iou_thresh=0.75, min_length=10, min_size=64, crop_scale=1.2,
         kernel_size=7, smooth=False, display=False, write_empty=False):
    ...
    for seq in seq_list:
        if smooth:
            seq.detections = smooth_detections_1euro(seq.detections, kernel_size)
        else:
            seq.finalize()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ks', '--kernel_size', default=7, type=int,
                        metavar='N', help='average kernel size')
    ...
    main(args.input, args.cache, args.output, args.iou_thresh, args.min_length, args.min_size, args.crop_scale,
         args.kernel_size, args.smooth, args.display, args.write_empty)
