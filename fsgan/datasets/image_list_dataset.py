...
def main(dataset='fake_detection.datasets.image_list_dataset.ImageListDataset', np_transforms=None,
    ...
    start = time.time()
    if isinstance(dataset,  datasets.image_list_dataset.ImageListDataset):
        for img, target in dataloader:
            ...
            for b in range(img.shape[0]):
                ...
                if cv2.waitKey(0) & 0xFF == ord('q'):
                    break
    elif isinstance(dataset,  datasets.image_list_dataset.ImagePairListDataset):
        for img1, img2, target in dataloader:
            ...
            for b in range(target.shape[0]):
                ...
                if cv2.waitKey(0) & 0xFF == ord('q'):
                    break
    elif isinstance(dataset,  datasets.image_list_dataset.ImageTripletListDataset):
